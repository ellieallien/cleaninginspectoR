...
easy_img.bar<-function(group,percent,error_min,error_max,size='tiny'){
  img(name = round(runif(1)*100000000000000000),width = 3,height = length(group)*0.2,plotfun = function(...){plot(reach_style_barchart(...))},group,percent,error_min,error_max)
}




resultbox<-function(graphic,title){

  graph<-graphic %>% div(class='result graphic')
  title<-title %>%   div(class='result title')
  return(div(paste(title,graph),class='result box'))
